...
pub fn Input(
    ...
    view! {
        <div class=format!("flex flex-col gap-2 w-full {}", if hidden { "hidden" } else { "" })>
            <label class="text-text-primary font-bold text-sm uppercase tracking-wider">{move || label.get()}</label>
            <div class="relative flex items-center">
                ...
                <Show when=move || !append.get().is_empty() fallback=|| ()>
                    <span class="absolute right-4 text-text-secondary font-bold">{move || append.get()}</span>
                </Show>
            </div>
